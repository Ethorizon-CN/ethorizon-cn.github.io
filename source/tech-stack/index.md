...
- **aplayer** - 音乐播放器
- **hexo-all-minifier** - 资源压缩插件，压缩 HTML / CSS / JS / 图片，优化加载速度
...
### 2.数据库&数据存储
- **Neon** - 用于 Waline 评论数据的持久化存储的 PostgreSQL 云数据库
- **MongoDB** - 用于 Qexo 博客管理的数据存储的 NoSQL 数据库

### 3. CI/CD 自动化& CDN 
- **GitHub Actions** - 执行构建、测试、部署&同步等任务的自动化工作流
- **NPM Mirror** - NPM 镜像源配置（如国内镜像），加速依赖安装过程
